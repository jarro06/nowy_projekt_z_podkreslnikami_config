...
def job_config = [
    job: [
        name: "nowy_projekt_z_podkreslnikami_Feature_Build",
        agent: "maven"
    ]
]
def config = [
...
ghe: [
        ...
        GitGroup: 'jarro06',
        RepoName: 'nowy_projekt_z_podkreslnikami',
        checks: [
...
def customLib = library identifier: 'nowy_projekt_z_podkreslnikami_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/nowy_projekt_z_podkreslnikami_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.CAMEL_CASE_nowy_projekt_z_podkreslnikami.new()
    ...
    stage('Prepare') {
      steps {
        ...
        script {
          ...
          jslGit.checkout( config, "jarro06", "nowy_projekt_z_podkreslnikami", branch_name)
          }
        }
      }
stage('Build') {
...
"""
def job = pipelineJob("nowy_projekt_z_podkreslnikami_Feature_Build");
...
job.with {
    ...
    authenticationToken('QHAkHo9kab3YLW0gMXP5FdSLm2Iacgzl')
