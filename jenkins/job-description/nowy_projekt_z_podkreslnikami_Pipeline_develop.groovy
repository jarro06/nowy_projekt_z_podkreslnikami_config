...
def job_config = [
        job: [
                name: "nowy_projekt_z_podkreslnikami_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "jarro06", "nowy_projekt_z_podkreslnikami", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("nowy_projekt_z_podkreslnikami_Pipeline_develop");
